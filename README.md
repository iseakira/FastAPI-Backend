...
今回修正した3つのバグから得られる教訓です。

---

バグ1・2: back_populates の名前ミスマッチ

# Shipment側

orders: list["Order"] = Relationship(back_populates="shipments") # ❌ "shipments"

# Order側

shipment: "Shipment" = Relationship(back_populates="orders") # ← フィールド名は "shipment"

教訓: リレーションは常に「相手のフィールド名」を書く

back_populates は「相手モデルにある自分を指しているフィールド名」を書くルール。
複数形・単数形を間違えやすいので、定義したらすぐ相手側も書いて対称になっているか確認する習慣をつける。

---

バグ3: モジュールスコープでのインスタンス生成

# models.py の末尾

shipment = Shipment() # ❌ インポート時に実行される

教訓: ORM モデルをモジュールトップレベルでインスタンス化しない

Python はモジュールをインポートした瞬間にトップレベルのコードを実行する。
ORM モデルのインスタンス化はマッパー設定を強制トリガーするため、まだ全モデルが揃っていない段階で実行されると整合性エラーになる。
「動作確認用に書いた一時コード」を消し忘れるのがよくある原因。

---

バグ4: 標準ライブラリと同名のクラスの混同

from pydantic import BaseModel, EmailStr, Field, Tag # ❌ pydantic.Tag を import
...
tags: list[Tag] # pydantic のユーティリティクラスを型として使ってしまった

教訓: 自分のモデル名と外部ライブラリのクラス名が衝突していないか確認する

Tag という名前は自プロジェクトのDBモデルにも存在し、pydantic にも存在する。
こういった名前衝突が起きたとき、import 文を見ただけでは意図が分かりにくくなる。
対策としては、衝突しそうな場合は as でエイリアスをつけるか、スキーマ専用クラス（TagRead）を使う。
...
│ バグ │ 根本原因 │
├──────────────────────────────┼────────────────────────────────────────────────────────────┤
│ back_populates ミス │ 手書きで対称性を保つ必要がある設計を、テストなしで放置した │
├──────────────────────────────┼────────────────────────────────────────────────────────────┤
│ トップレベルのインスタンス化 │ 一時コードの削除漏れ │
├──────────────────────────────┼────────────────────────────────────────────────────────────┤
│ pydantic.Tag の誤用 │ 名前が同じでも別物であることに気づかなかった │
└──────────────────────────────┴────────────────────────────────────────────────────────────┘

全バグに共通する教訓: 起動時エラーはテストで早期検出できる。
fastapi dev で初めて気づくのではなく、import が通るかだけでも CI で確認する仕組みがあれば全て事前に防げた。
